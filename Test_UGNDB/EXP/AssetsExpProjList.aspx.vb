' ************************************************************************************************
' Name:	AssetsExpProjList.aspx.vb
...
' 10/24/2012    LRey            Added an image button to IOR's if submitted.
' 02/05/2014	LRey        Replaced DeptOrCostCenter with new ERP values.
' ************************************************************************************************
Partial Class EXP_AssetsExpProjList
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            m.ContentLabel = "Property Plant Equipment (Asset) Search"
            ...
            If ctl IsNot Nothing Then
                ...
                lbl.Text = "<a href='../Home.aspx'><b>Home</b></a> > <b>Spending Requests</b> > Property Plant Equipment (Asset) Search"
                lbl.Visible = True
            End If
            ...
            If HttpContext.Current.Request.QueryString("pAprv") <> "" Then
                ...
                HyperLink1.NavigateUrl = "~/EXP/crExpProjAssetsApproval.aspx?pProjNo=" & ViewState("pProjNo") & "&pAprv=1"
            Else
                ViewState("pAprv") = 0
            End If
            ...
            If Not Page.IsPostBack Then
                ...
                ''******
                If Not Request.Cookies("EXPA_ProjNo") Is Nothing Then
                    txtProjectNo.Text = Server.HtmlEncode(Request.Cookies("EXPA_ProjNo").Value)
                    ViewState("sProjNo") = Server.HtmlEncode(Request.Cookies("EXPA_ProjNo").Value)
                End If


                If Not Request.Cookies("EXPA_SupProjNo") Is Nothing Then
                    txtSupProjectNo.Text = Server.HtmlEncode(Request.Cookies("EXPA_SupProjNo").Value)
                    ViewState("sSupProjNo") = Server.HtmlEncode(Request.Cookies("EXPA_SupProjNo").Value)
                End If

                If Not Request.Cookies("EXPA_ProjTitle") Is Nothing Then
                    txtProjectTitle.Text = Server.HtmlEncode(Request.Cookies("EXPA_ProjTitle").Value)
                    ViewState("sProjTitle") = Server.HtmlEncode(Request.Cookies("EXPA_ProjTitle").Value)
                End If

                If Not Request.Cookies("EXPA_UGNFacility") Is Nothing Then
                    ddUGNFacility.SelectedValue = Server.HtmlEncode(Request.Cookies("EXPA_UGNFacility").Value)
                    ViewState("sUGNFacility") = Server.HtmlEncode(Request.Cookies("EXPA_UGNFacility").Value)
                End If

                If Not Request.Cookies("EXPA_PLDRID") Is Nothing Then
                    ddProjectLeader.SelectedValue = Server.HtmlEncode(Request.Cookies("EXPA_PLDRID").Value)
                    ViewState("sPLDRID") = Server.HtmlEncode(Request.Cookies("EXPA_PLDRID").Value)
                End If

                If Not Request.Cookies("EXPA_DEPT") Is Nothing Then
                    txtDepartment.Text = Server.HtmlEncode(Request.Cookies("EXPA_DEPT").Value)
                    ViewState("sDept") = Server.HtmlEncode(Request.Cookies("EXPA_DEPT").Value)
                End If

                If Not Request.Cookies("EXPA_CatID") Is Nothing Then
                    ddCategory.SelectedValue = Server.HtmlEncode(Request.Cookies("EXPA_CatID").Value)
                    ViewState("sCatID") = Server.HtmlEncode(Request.Cookies("EXPA_CatID").Value)
                End If

                If Not Request.Cookies("EXPA_CapCls") Is Nothing Then
                    ddCapitalSpending.SelectedValue = Server.HtmlEncode(Request.Cookies("EXPA_CapCls").Value)
                    ViewState("sCapCls") = Server.HtmlEncode(Request.Cookies("EXPA_CapCls").Value)
                End If

                If Not Request.Cookies("EXPA_PStatus") Is Nothing Then
                    ddProjectStatus.SelectedValue = Server.HtmlEncode(Request.Cookies("EXPA_PStatus").Value)
                    ViewState("sPStatus") = Server.HtmlEncode(Request.Cookies("EXPA_PStatus").Value)
                End If
            ...
            Else
                ...
                ViewState("sCatID") = ddCategory.SelectedValue
                ViewState("sCapCls") = ddCapitalSpending.SelectedValue
                ViewState("sPStatus") = ddProjectStatus.SelectedValue
            End If
...
#Region "Form Level Security using Roles &/or Subscriptions"
    Protected Sub CheckRights()
        ...
        Try
            ...
            Dim iFormID As Integer = 101 'Expensed Project Form ID
...
#End Region 'EOF Security
    ...
    Private Sub BindData()
        Try
            ...
            ds = EXPModule.GetExpProjAssets(ViewState("sProjNo"), ViewState("sSupProjNo"), ViewState("sProjTitle"), ViewState("sUGNFacility"), IIf(ViewState("sPLDRID") = Nothing, 0, ViewState("sPLDRID")), ViewState("sDept"), IIf(ViewState("sCatID") = Nothing, 0, ViewState("sCatID")), ViewState("sCapCls"), ViewState("sPStatus"))

            If ds IsNot Nothing Then
                If ds.Tables.Count > 0 Then
                    rpAssetsExpProj.DataSource = ds
                    rpAssetsExpProj.DataBind()
                    ...
                    rpAssetsExpProj.DataSource = objPds
                    rpAssetsExpProj.DataBind()
                    ...
                    Else
                        ...
                        lblToRec.Text = rpAssetsExpProj.Items.Count
                    End If
...
#End Region 'EOF Paging Routine
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        Try
            ...
            Response.Cookies("EXPA_ProjNo").Value = txtProjectNo.Text
            Response.Cookies("EXPA_SupProjNo").Value = txtSupProjectNo.Text
            Response.Cookies("EXPA_ProjTitle").Value = txtProjectTitle.Text
            Response.Cookies("EXPA_UGNFacility").Value = ddUGNFacility.SelectedValue
            Response.Cookies("EXPA_PLDRID").Value = ddProjectLeader.SelectedValue
            Response.Cookies("EXPA_DEPT").Value = txtDepartment.Text
            Response.Cookies("EXPA_CatID").Value = ddCategory.SelectedValue
            Response.Cookies("EXPA_CapCls").Value = ddCapitalSpending.SelectedValue
            Response.Cookies("EXPA_PStatus").Value = ddProjectStatus.SelectedValue
    ...
    Protected Sub btnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click
        Try
            EXPModule.DeleteAssetsExpProjCookies()
            HttpContext.Current.Session("sessionExpCurrentPage") = Nothing
    ...
    Public Function GoToAppend(ByVal ProjectNo As String, ByVal ParentProjectNo As String) As String
        ...
        If ParentProjectNo = Nothing Or ParentProjectNo Is DBNull.Value Then
            Return "AssetsExpProj.aspx?pProjNo=&pPrntProjNo=" & ProjectNo
        Else
            Return ""
        End If
