...
Partial Class Workflow_Team_Member_WorkFlow
    Inherits System.Web.UI.Page
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            Dim m As ASP.masterpage_master = Master
            m.PageTitle = "UGN, Inc."
            m.ContentLabel = "Team Member Work Flow"
            ''***********************************************
            ...
            If Not mpTextBox Is Nothing Then
                mpTextBox.Text = "<a href='../Home.aspx'><b>Home</b></a> > <b>Work Flow</b> > Team Member Work Flow"
            ...
            If Not Page.IsPostBack Then
                ...
                ''******
                If Not Request.Cookies("WF_TeamMember") Is Nothing Then
                    ddTeamMember.SelectedValue = Server.HtmlEncode(Request.Cookies("WF_TeamMember").Value)
                    ViewState("sTeamMember") = Server.HtmlEncode(Request.Cookies("WF_TeamMember").Value)
                End If

                If Not Request.Cookies("WF_Subscription") Is Nothing Then
                    ddSubscription.SelectedValue = Server.HtmlEncode(Request.Cookies("WF_Subscription").Value)
                    ViewState("sSubscription") = Server.HtmlEncode(Request.Cookies("WF_Subscription").Value)
                End If

            Else
                ViewState("sTeamMember") = ddTeamMember.SelectedValue
                ViewState("sSubscription") = ddSubscription.SelectedValue
            End If
...
#Region "Form Level Security using Roles &/or Subscriptions"
    Protected Sub CheckRights()
        ...
        Try
            ...
            ''*******
            gvWorkFlow.Columns(5).Visible = False
            gvWorkFlow.Columns(6).Visible = False
            ...
            Dim iFormID As Integer = 30 'Team Member Work Flow form id
                            ...
                            If dsRoleForm.Tables.Count And dsRoleForm.Tables(0).Rows.Count > 0 Then
                                ...
                                Select Case iRoleID
                                    Case 11 '*** UGNAdmin: Full Access
                                        gvWorkFlow.Columns(5).Visible = True
                                        gvWorkFlow.Columns(6).Visible = True
...
#End Region
    ...
    Protected Sub BindCriteria()
        ...
        ''bind existing data to drop down Subscription control for selection criteria for search
        ds = commonFunctions.GetSubscriptions("")
        If (ds.Tables.Item(0).Rows.Count > 0) Then
            ddSubscription.DataSource = ds
            ddSubscription.DataTextField = ds.Tables(0).Columns("Subscription").ColumnName.ToString()
            ddSubscription.DataValueField = ds.Tables(0).Columns("SubscriptionID").ColumnName.ToString()
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        ''******
        Response.Cookies("WF_TeamMember").Value = ddTeamMember.SelectedValue
        Response.Cookies("WF_Subscription").Value = ddSubscription.SelectedValue

        ''******
        '' Redirect to the Team Member Backups List page
        ''******
        Response.Redirect("Team_Member_WorkFlow.aspx?sTeamMember=" & ddTeamMember.SelectedValue & "&sSubscription=" & ddSubscription.SelectedValue, True)

    End Sub
    Protected Sub btnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click
        ddTeamMember.SelectedValue = ""
        ddSubscription.SelectedValue = ""

        ''******
        '' Delete cookies in search parameters.
        ''******
        WorkFlowModule.DeleteWFCookies_TeamMemberBackupSearch()

        ''******
        '' Redirect to the Team Member Backups List page
        ''******
        Response.Redirect("Team_Member_WorkFlow.aspx", True)
    End Sub
    Protected Sub gvWorkFlow_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvWorkFlow.RowCommand
        ...
        Dim Subscription As DropDownList
        Dim BackupTeamMember As DropDownList
        Dim DeptInCharge As DropDownList
        ...
        If e.CommandName = "Insert" Then
            ...
            Subscription = CType(gvWorkFlow.FooterRow.FindControl("ddSubscription"), DropDownList)
            odsWorkFlow.InsertParameters("SubscriptionID").DefaultValue = Subscription.SelectedValue

            BackupTeamMember = CType(gvWorkFlow.FooterRow.FindControl("ddBackupTeamMember"), DropDownList)
            odsWorkFlow.InsertParameters("BackupTeamMemberID").DefaultValue = BackupTeamMember.SelectedValue

            DeptInCharge = CType(gvWorkFlow.FooterRow.FindControl("ddDeptInCharge"), DropDownList)
            odsWorkFlow.InsertParameters("DeptInChargeTMID").DefaultValue = DeptInCharge.SelectedValue
        ...
        If e.CommandName = "Undo" Then
            ...
            BackupTeamMember = CType(gvWorkFlow.FooterRow.FindControl("ddBackupTeamMember"), DropDownList)
            BackupTeamMember.ClearSelection()
            BackupTeamMember.Items.Add("")
            BackupTeamMember.SelectedValue = ""

            DeptInCharge = CType(gvWorkFlow.FooterRow.FindControl("ddDeptInCharge"), DropDownList)
            DeptInCharge.ClearSelection()
            DeptInCharge.Items.Add("")
            DeptInCharge.SelectedValue = ""
    ...
    Protected Sub gvWorkFlow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvWorkFlow.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If imgBtn IsNot Nothing Then
                Dim db As ImageButton = CType(e.Row.Cells(5).Controls(3), ImageButton)

                ' Get information about the product bound to the row
                If db.CommandName = "Delete" Then
                    Dim TeamMember As WorkFlow.WorkFlowRow = CType(CType(e.Row.DataItem, System.Data.DataRowView).Row, WorkFlow.WorkFlowRow)

                    db.OnClientClick = String.Format("return confirm('Are you certain you want to delete Team Member (" & DataBinder.Eval(e.Row.DataItem, "TeamMemberName") & ")  Subscription (" & DataBinder.Eval(e.Row.DataItem, "Subscription") & ")?');")
...
#Region "Insert Empty GridView Work-Around"
    ...
    Protected Sub odsWorkFlow_Selected(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceStatusEventArgs) Handles odsWorkFlow.Selected
        ...
        Dim dt As WorkFlow.WorkFlowDataTable = CType(e.ReturnValue, WorkFlow.WorkFlowDataTable)
