...
''*****************************************************************************
'' Name    : MenuMaintenance.aspx.vb
'' Purpose : Displays and updates the Menu_Maint table in the UGNDB database.
''
'' Date        Author     Modifications	    
'' 07/15/2008  LRey       Created .Net application
'' 
...
Partial Class Security_MenuMaintenance
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If ctl IsNot Nothing Then
                ...
                lbl.Text = _
                    "<a href='../Home.aspx'><b>Home</b></a> > <b> Security </b> > Menu"
                lbl.Visible = True
            End If
...
#Region "Event Handlers"
    ...
    Protected Sub gvMenu_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvMenu.RowCommand
        Try
            ...
            If e.CommandName = "Edit" Then
                gvMenu.ShowFooter = False
            Else
                gvMenu.ShowFooter = True
            End If

            If e.CommandName = "InsertCustom" AndAlso Page.IsValid() Then
                ' Get the data for inserting a new record
                Dim strMenuName As String = _
                    CType(gvMenu.FooterRow.FindControl("txtMenuNameInsert"), TextBox).Text
                Dim blnObsolete As Boolean = _
                    CType(gvMenu.FooterRow.FindControl("chkObsoleteInsert"), CheckBox).Checked
                InsertNewRow(strMenuName, blnObsolete)
    ...
    Protected Sub ibtnUpdate_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        Try
            ...
            Dim intMenuID As Integer = _
                CType(CType(gvr.FindControl("lblMenuIDPreEdit"), Label).Text, Integer)
            Dim strMenuName As String = _
                CType(gvr.FindControl("txtMenuNameEdit"), TextBox).Text
            Dim blnObsolete As Boolean = _
                CType(gvr.FindControl("chkObsoleteEdit"), CheckBox).Checked
            Dim blnSuccess As Boolean = _
                UpdateRow(intMenuID, strMenuName, blnObsolete)
...
#Region "Private Methods"
    ...
    ''' <remarks></remarks>
    Private Sub InsertNewRow(ByVal MenuName As String, ByVal obsolete As Boolean)
        Try
            ' Find out if the MenuName already exists in another record.
            Dim intRecordCount As Integer = SecurityModule.GetMenuCount(Nothing, MenuName, Nothing)
            If intRecordCount < 0 Then
                ' Error was encountered
                DisplayStatus("Error was encounter. Record was not inserted.", True)
                Exit Sub
            ElseIf intRecordCount > 0 Then
                ' The MenuName already exists.
                ' Do not insert it into the table.
                DisplayStatus("Duplicate Menu Name found. New record was not inserted.", True)
                Exit Sub
            End If

            ' Insert the new record
            Dim intNewID As Integer = SecurityModule.InsertMenu( _
                MenuName, obsolete)
            If intNewID = -1 Then
    ...
    ''' <summary>
    ''' Updates a record in the Menu_Maint table
    ''' </summary>
    ''' <param name="MenuID">The MenuID of the record to update</param>
    ''' <param name="MenuName">New value of MenuName</param>
    ''' <param name="Obsolete">New value of Obsolete</param>
    ''' <remarks></remarks>
    Private Function UpdateRow(ByVal MenuID As Integer, _
        ByVal MenuName As String, ByVal Obsolete As Boolean) As Boolean

        Dim blnSuccess As Boolean = False
        Try
            ' Check the MenuName to make sure it is not already used in another record.
            Dim ds As DataSet = New DataSet()
            ds = SecurityModule.GetMenu(Nothing, MenuName, Nothing, Nothing)
            For Each dr As DataRow In ds.Tables(0).Rows
                Dim strIdInTable As String = dr.Item("MenuID").ToString()
                If Not strIdInTable.Equals(MenuID.ToString()) Then
                    ' A match was found.
                    ' Report error, and cancel the update
                    DisplayStatus("Menu Name '" & MenuName & _
                        "' already exists for MenuID = " & strIdInTable, True)
                    Return False
                End If
            Next

            ' Update the record
            blnSuccess = SecurityModule.UpdateMenu(MenuID, MenuName, Obsolete)
            If blnSuccess Then
                DisplayStatus("")
            Else
...
#Region "Insert Empty GridView Work-Around"
    ...
    Protected Sub odsMenu_Selected(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceStatusEventArgs) Handles odsMenu.Selected
        Try
            ' From Andrew Robinson's Insert Empty GridView solution
            ' http://blog.binaryocean.com/2006/01/05/InsertRowsWithAGridView.aspx
        ...
        Catch ex As Exception
            ...
            UGNErrorTrapping.UpdateUGNErrorLog(mb.Name & ":" & ex.Message, _
                System.Web.HttpContext.Current.Request.Url.AbsolutePath)
        End Try
    End Sub ' odsMenu_Selected

    Protected Sub gvMenu_RowCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvMenu.RowCreated
        Try
            ' From Andrew Robinson's Insert Empty GridView solution
            ' http://blog.binaryocean.com/2006/01/05/InsertRowsWithAGridView.aspx
